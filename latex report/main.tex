...
Cette implémentation du Mini-HPL parallèle illustre les principes fondamentaux du benchmark HPL : élimination de Gauss avec pivotage partiel, distribution bloc-cyclique des données et coordination via communications MPI. Les opérations collectives (\texttt{MPI\_Allreduce}, \texttt{MPI\_Bcast}, \texttt{MPI\_Reduce}) assurent la cohérence globale du calcul, tandis que le coût dominant reste en $\mathcal{O}(N^3)$. La vérification par la résiduelle normée garantit la validité numérique des résultats. Ce Mini-HPL constitue ainsi une base pédagogique pour comprendre le fonctionnement interne d'un benchmark HPC parallèle. Le code source complet est reproduit en annexe~\ref{app:source}.
...
% ==========================================================
\appendix
\section{Code source complet : Mini-HPL}
\label{app:source}

Le fichier \texttt{mini\_hpl.c} ci-dessous constitue l'implémentation complète du Mini-HPL parallèle avec MPI, tel que décrit en section~\ref{sec:mini-hpl}. Il est fourni en tant que livrable séparé avec ce rapport.

\lstinputlisting[
  style=cstyle,
  caption={mini\_hpl.c --- Code source complet},
  label={lst:full-source}
]{../project/mini_hpl.c.txt}
