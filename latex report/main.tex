...
Les expériences ont été réalisées sur le supercalculateur \textbf{Toubkal}, opéré par l'Université Mohammed VI Polytechnique (Maroc). Les nœuds GPU utilisés sont équipés de :
...
Le pic théorique FP64 de l'A100 est calculé à partir du nombre de cœurs CUDA FP64, du facteur FMA (\textit{Fused Multiply-Add}, chaque opération FMA compte pour 2 opérations flottantes), de la fréquence boost, et du facteur Tensor Core :

\[
\text{Peak}_{A100} = \underbrace{3\,456}_{\substack{\text{cœurs} \\ \text{FP64}}} \times \underbrace{2}_{\text{FMA}} \times \underbrace{1{,}41 \text{ GHz}}_{\text{freq. boost}} = 9{,}7 \text{ TFLOPS (cœurs CUDA)}
\]

Le benchmark HPL utilise les \textbf{Tensor Cores} pour les opérations DGEMM (produits de matrices denses). Sur l'A100, les Tensor Cores de 3\textsuperscript{e} génération doublent le débit FP64 :

\[
\text{Peak}_{A100}^{\text{TC}} = 9{,}7 \times 2 = 19{,}5 \text{ TFLOPS}
\]

Ce qui correspond au pic officiel NVIDIA pour l'A100~\cite{nvidia_a100}.

Efficacité maximale atteinte (à $N = 100\,000$) :

\[
\eta_{A100} = \frac{17\,860}{19\,500} \times 100 \approx 91{,}6\%
\]

Cette efficacité élevée confirme que HPL est un benchmark \textit{compute-bound} : la majorité du temps est consacrée aux opérations DGEMM qui exploitent pleinement les Tensor Cores du GPU. Tous les tests ont passé la vérification du résidu ($\|Ax - b\|_\infty / (\varepsilon \cdot (\|A\|_\infty \cdot \|x\|_\infty + \|b\|_\infty) \cdot N) < 16{,}0$), confirmant la validité numérique des résultats.
...
Le pic théorique FP64 du H100 avec Tensor Cores (4\textsuperscript{e} génération) est de \textbf{51 TFLOPS} pour la variante PCIe, selon la fiche technique officielle NVIDIA~\cite{nvidia_h100}. Ce pic est obtenu par le même principe que pour l'A100 : les cœurs FP64 (ici 7\,296 pour le H100 PCIe, soit 64 par SM $\times$ 114 SM) exécutent des opérations FMA, et les Tensor Cores doublent le débit effectif.

\[
\text{Peak}_{H100}^{\text{TC}} = 51 \text{ TFLOPS (FP64 Tensor Core, variante PCIe)}
\]

La variante SXM du H100 atteint 67 TFLOPS grâce à davantage de SM actifs (132 vs 114) et une fréquence boost plus élevée (1\,830 MHz vs 1\,620 MHz).

Efficacité maximale atteinte (à $N = 100\,000$) :

\[
\eta_{H100} = \frac{45\,110}{51\,000} \times 100 \approx 88{,}5\%
\]

L'efficacité du H100 est légèrement inférieure à celle de l'A100 (88{,}5\% vs 91{,}6\%). Le H100 possède 2{,}4$\times$ plus de cœurs FP64 CUDA que l'A100 (8\,448 vs 3\,456), ce qui rend plus difficile la saturation complète de toutes les unités de calcul pour une même taille de problème. Pour atteindre une efficacité comparable, des valeurs de $N$ encore plus grandes seraient nécessaires.
...
Le ratio converge vers $\approx 2{,}5\times$ pour les grandes tailles de problème. Cela est cohérent avec le rapport des pics théoriques FP64 (Tensor Cores) :

\[
\frac{\text{Peak}_{H100}^{\text{TC}}}{\text{Peak}_{A100}^{\text{TC}}} = \frac{51\,000}{19\,500} \approx 2{,}62
\]

Le facteur mesuré (2{,}53$\times$) est proche du facteur théorique (2{,}62$\times$), avec un écart de seulement $\approx 3\%$. Ce faible écart confirme que HPL exploite efficacement les deux architectures et que le ratio de performance mesuré reflète bien le rapport des capacités matérielles.
...
\begin{enumerate}
    \item \textbf{HPL est un benchmark compute-bound :} Les efficacités mesurées (88--92\%) confirment que la performance est principalement limitée par la puissance de calcul et non par la bande passante mémoire ou les communications.
    ...
    \item \textbf{Le H100 offre un gain de $\approx 2{,}5\times$ sur l'A100 :} Ce ratio est cohérent avec le rapport des pics théoriques FP64 Tensor Core ($2{,}62\times$), avec un écart de seulement 3\%.
...
\bibitem{nvidia_a100}
NVIDIA Corporation, \textit{NVIDIA A100 Tensor Core GPU --- Datasheet}, 2020. Pic FP64 officiel : 19{,}5 TFLOPS (Tensor Core). \url{https://www.nvidia.com/en-us/data-center/a100/}

\bibitem{nvidia_h100}
NVIDIA Corporation, \textit{NVIDIA H100 Tensor Core GPU --- Datasheet}, 2022. Pic FP64 officiel : 51 TFLOPS (Tensor Core, PCIe) / 67 TFLOPS (SXM). \url{https://www.nvidia.com/en-us/data-center/h100/}
